...
public class WhisperIntegration : MonoBehaviour
{
    ...
    {
        // Save audio clip to WAV file in the Assets folder
        SaveAudioClipToWav(audioClip, relativeFilePath);

        Debug.Log("WAV file saved at: " + relativeFilePath);
        ...
        byte[] audioData = File.ReadAllBytes(relativeFilePath);
        Debug.Log($"Audio file size: {audioData.Length} bytes");

        // Create a form and attach the file
        WWWForm form = new WWWForm();
        form.AddBinaryData("audio", audioData, "temp_audio.wav", "audio/wav");
