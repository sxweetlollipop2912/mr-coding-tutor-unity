...
public class YappingHandler : MonoBehaviour
{
    ...
    private IEnumerator YappingRoutine()
    {
        ...
        while (isYapping && !shouldStopYapping)
        {
            ...
            {
                Debug.Log("[YappingHandler] Waiting 3 seconds before next phrase");
    ...
    private IEnumerator PlayAudio(string filePath)
    {
        ...
        try
        {
            using (var request = UnityWebRequestMultimedia.GetAudioClip(uriPath, audioType))
            {
                ...
                if (request.result == UnityWebRequest.Result.Success)
                {
                    ...
                    {
                        Debug.LogError(
                            $"[YappingHandler] AudioClip is null after loading: {filePath}"
                        );
                        yield break;
                    }
                ...
                else
                {
                    ...
                    StartCoroutine(PlayAudioAlternative(filePath));
                }
            }
        }
        ...
        {
            Debug.LogError(
                $"[YappingHandler] Exception loading audio: {e.Message}\n{e.StackTrace}"
            );
            // Try an alternative approach for local files
            StartCoroutine(PlayAudioAlternative(filePath));
        }
    ...
    private IEnumerator PlayAudioAlternative(string filePath)
    {
        ...
        if (Path.GetExtension(filePath).ToLower() == ".wav")
        {
            try
            {
                ...
                // Use WWW to load the audio (deprecated but sometimes works when WebRequest fails)
...
#pragma warning restore 0618
            ...
            {
                Debug.LogError(
                    $"[YappingHandler] Exception in alternative audio loading: {e.Message}"
                );
            }
        }
        else
