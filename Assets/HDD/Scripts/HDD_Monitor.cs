...
namespace HyperDesktopDuplication
{
    public class HDD_Monitor : MonoBehaviour
    {
        ...
        bool destroyed;
        bool useCustomCursor = false; // Flag to control cursor texture processing
        ...
        public int width =>
            this.portrait ? this.info.Bottom - this.info.Top : this.info.Right - this.info.Left;
        public int height =>
            this.portrait ? this.info.Right - this.info.Left : this.info.Bottom - this.info.Top;

        /// <summary>
        /// Sets whether to use custom cursor texture instead of processing captured pointer shape data.
        /// </summary>
        public void SetUseCustomCursor(bool useCustom)
        {
            this.useCustomCursor = useCustom;
        }
        ...
        )
        {
            ...
            this.texture = new Texture2D(
                this.portrait ? this.pixelHeight : this.pixelWidth,
                this.portrait ? this.pixelWidth : this.pixelHeight,
                TextureFormat.BGRA32,
                false
            );
            this.desktopRenderer.GetComponent<Renderer>().material.mainTexture = this.texture;
            Logger.Log(
                $"display {this.id}: texture created with size: {this.pixelWidth}x{this.pixelHeight}"
            );
            this.desktopRenderer.transform.localScale = new Vector3(this.width, this.height, 1); // resize to a proper size
            this.desktopRenderer.transform.localRotation = Quaternion.Euler(
                0,
                0,
                this.rotation * 90 - 90
            ); // rotate to a proper angle
        ...
        async void CreateCapture()
        {
            ...
            await client.CreateCaptureAsync(
                new Shremdup.CreateCaptureRequest
                {
                    Id = (uint)this.id,
                    Name = filename,
                    Open = false,
            ...
            {
                Logger.Log(
                    $"display {this.id}, filename {filename}, access {FILE_MAP_ALL_ACCESS}: OpenFileMapping() failed: {Marshal.GetLastWin32Error()}"
                );
                return;
            }
        ...
        async void TakeCapture()
        {
            ...
            try
            {
                var res = await client.TakeCaptureAsync(
                    new Shremdup.TakeCaptureRequest { Id = (uint)this.id }
                );
                ...
                {
                    // update mouse position (always keep this for position sync)
                    ...
                    mouse.localPosition = new Vector3(
                        -this.pixelWidth / 2 + pos.X + this.mouse.transform.localScale.x / 2,
                        -this.pixelHeight / 2 + pos.Y + this.mouse.transform.localScale.y / 2,
                        -0.001f
                    );
                    this.mousePixelPosition = (pos.X, pos.Y);
                }

                // Skip pointer shape processing if using custom cursor
                if (!useCustomCursor && res.PointerShape != null)
                {
                    // update mouse shape
                    ...
                    switch (shape.ShapeType)
                    {
                        case 1:
                        {
                            ...
                            var cursorTexture = new Texture2D(
                                (int)shape.Width,
                                (int)shape.Height / 2,
                            ...
                            this.mouse.transform.localScale = new Vector3(
                                shape.Width,
                                shape.Height / 2,
                                1
                            );
                        ...
                        {
                            Logger.Log(
                                $"display {this.id}: unknown pointer shape type {shape.ShapeType}"
                            );
                            break;
                        }
                    }
                ...
                // Skip masked color processing if using custom cursor
                if (
                    !useCustomCursor
                    && this.pointerShapeCache.ShapeType == 4
                    && (
                        res.DesktopUpdated
                        || res.PointerPosition != null
                        || res.PointerShape != null
                    )
                )
                {
                    // DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MASKED_COLOR
        ...
        void UpdatePointerShapeWithMask()
        {
            ...
            if (
                cursorTexture == null
                || cursorTexture.width != shape.Width
                || cursorTexture.height != shape.Height
            )
            ...
            for (var i = 0; i < raw.Length; i += 4)
            {
                ...
                var desktopX =
                    (cursorPixelIndex % (int)shape.Width) + this.mousePixelPosition.Item1;
                var desktopY =
                    (cursorPixelIndex / (int)shape.Width) + this.mousePixelPosition.Item2;
        ...
        void Update()
        {
            ...
            if (this.state == State.TakeCaptureDone && this.desktopRenderer.visible)
                this.TakeCapture();
        }
